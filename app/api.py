...
from src.embeddings import get_embedding_model, get_or_compute_article_embeddings, encode_texts
from src.config import HF_EMBED_MODEL_NAME, EMBEDDINGS_CACHE_PATH
...
# Cache global embeddings (chargés à la demande)
embedding_model = None
article_embeddings = None


def _ensure_embeddings_loaded():
    """Charge le modèle MiniLM et les embeddings articles une seule fois, avec fallback silencieux."""
    global embedding_model, article_embeddings
    try:
        ...
        return embedding_model, article_embeddings
    except Exception as e:
        logger.warning(f"Embeddings unavailable, fallback TF-IDF: {e}")
        return None, None
        ...
        if tags:
            ...
            if tag_list:
                ...
                if not profile_text.strip():
                    ...
                    # Try embeddings first, fallback to TF-IDF
                    model, art_emb = _ensure_embeddings_loaded()
                    if model is not None and art_emb is not None:
                        try:
                            profile_emb = encode_texts([profile_text], model, normalize=True)
                            recs_df = recommend_for_profile_emb(profile_emb, art_emb, articles_df, top_k=10)
                        except Exception as e:
                            logger.warning(f"Explore page embeddings failed, fallback TF-IDF: {e}")
...
def article_page(article_id: str, request: Request):
    ...
    try:
        ...
        if model is not None and art_emb is not None:
            try:
                recs = recommend_similar_to_article_emb(article_id, art_emb, articles_df, top_k=5)
            except Exception as e:
                logger.warning(f"Article page embeddings failed, fallback TF-IDF: {e}")
...
def api_interact_like(req: LikeRequest, representation: str = "embeddings"):
    ...
    # Route embeddings si demandé et disponible
    if representation == "embeddings":
        model, art_emb = _ensure_embeddings_loaded()
        if model is not None and art_emb is not None:
            try:
                profile_emb = encode_texts([profile_text], model, normalize=True)
                v_updated = update_profile_with_likes_emb(profile_emb, [req.article_id], art_emb, articles_df)
                recs = recommend_for_profile_emb(
                    v_updated,
                    art_emb,
                    articles_df,
                    top_k=5,
                    exclude_ids={req.article_id},
                )
                ...
                logger.warning(f"Embeddings like route fallback to TF-IDF: {e}")

    # 2. Vectorize base profile (TF-IDF)
    v_profile = vectorizer.transform([profile_text])

    # 3. Update with the new like (TF-IDF)
    v_updated = update_profile_with_likes(v_profile, [req.article_id], X_tfidf, articles_df)

    # 4. Recommend (exclude the liked article) (TF-IDF)
...
def api_recommend_profile(req: ProfileRequest, representation: str = "tfidf"):
    profile_text = build_profile_text(req.prefs, profile_kw_df)

    # Route embeddings si demandé
    if representation == "embeddings":
        model, art_emb = _ensure_embeddings_loaded()
        if model is not None and art_emb is not None:
            try:
                profile_emb = encode_texts([profile_text], model, normalize=True)
                if req.liked_ids:
                    profile_emb = update_profile_with_likes_emb(
                        profile_emb, req.liked_ids, art_emb, articles_df
                    )
                recs = recommend_for_profile_emb(profile_emb, art_emb, articles_df, top_k=5)
                results = recs.to_dict(orient="records")
                for r in results:
                    r["image_url"] = get_article_image(r.get("field"))
                return results
            except Exception as e:
                logger.warning(f"Embeddings profile route fallback to TF-IDF: {e}")

    # TF-IDF (défaut ou fallback)
    v_profile = profile_to_vector(profile_text, vectorizer)
...
def api_recommend_similar(article_id: str, top_k: int = 5, representation: str = "tfidf"):
    if representation == "embeddings":
        model, art_emb = _ensure_embeddings_loaded()
        if model is not None and art_emb is not None:
            try:
                recs = recommend_similar_to_article_emb(article_id, art_emb, articles_df, top_k)
                results = recs.to_dict(orient="records")
                for r in results:
                    r["image_url"] = get_article_image(r.get("field"))
                return results
            except Exception as e:
                logger.warning(f"Embeddings similar route fallback to TF-IDF: {e}")
